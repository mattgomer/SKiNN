...
class CIPSskip(nn.Module):
    def __init__(self, size=256, hidden_size=512, n_mlp=5, style_dim=512, lr_mlp=0.01,
...
if __name__ == "__main__":
    generator = CIPSskip().cuda()
    aa = convert_to_coord_format(1, 275, 275, integer_values=False)
    sample_z = torch.randn(1, 512)
    out ,_ = generator(aa.cuda(), [sample_z.cuda()])
